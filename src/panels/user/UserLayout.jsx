...
import { ShoppingBag, Search, User, Store, Menu, X, Package } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-bazaar-bg font-bazaar text-bazaar-ink">
      ...
      <header className="bg-bazaar-primary sticky top-0 z-30 shadow-md">
          ...
          <nav className="flex items-center gap-3 md:gap-5 text-sm shrink-0 text-white">
            {user ? (
              <>
                ...
                <Link to="/account" className="flex flex-col items-center leading-tight hover:text-bazaar-gold">
                  <User size={18} />
                  <span className="text-[10px] mt-0.5 hidden sm:block">Account</span>
                </Link>
              </>
            ) : (
        ...
        {/* Category row — real subcategories on hover, Meesho-style */}
        <CategoryMegaMenu />
        ...
        {menuOpen && (
          <div className="lg:hidden bg-white px-4 py-4 space-y-3 border-t border-bazaar-border">
            ...
            <div className="pt-3 border-t border-bazaar-border space-y-3">
              {user ? (
                <>
                  <Link to="/account" onClick={() => setMenuOpen(false)} className="block text-sm">My account</Link>
